	...
	describe('search', () => {
		...
		describe('searchable fields', () => {
			...
			it('should be searchable by any name variation', async () => {
				...
				//Typo in below test; Was Joe changed to Joey
				expect(service.search('Joey').length).to.equal(1); // nick name
		...
		describe('formatContact', () => {
			it('should format contact to expected format', async () => {
				const contact = {
					firstName: 'First',
					lastName: 'Last',
					nickName: 'Middle',
				...
				let results = service.formatContact(contact);
		...
		describe('formatPhoneNumber', () => {
			it('should format phone numbers to expected format', async () => {
				const phoneNumbers = [ '314-555-0000', '+13145551234' ];

				let results = service.formatPhoneNumber(phoneNumbers);
				expect(results[0]).to.equal('(314) 555-0000');
				expect(results[1]).to.equal('(314) 555-1234');
			});
		});
	});
});
